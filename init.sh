...
echo "🚀 Initializing CKAD Lab Environment..."
echo ""
...
echo "🔧 Creating environment variables and helper functions..."
echo ""
...
if [ -n "$SHELL_CONFIG" ]; then
    if ! grep -q "CKAD Lab Helper Functions" "$SHELL_CONFIG" 2>/dev/null; then
        echo "📝 Adding functions to $SHELL_CONFIG..."
...
EOF
        ...
        echo "ℹ️  Functions already in $SHELL_CONFIG (updated QUESTIONS_DIR)"
    fi
fi

# Always source in current session to make functions available immediately
echo "🔄 Loading functions in current session..."
export QUESTIONS_DIR="$QUESTIONS_DIR"
...
if type exec-lab &> /dev/null && type check-lab &> /dev/null; then
    ...
    echo "⚠️  Functions may not be loaded. Try: source $FUNCTIONS_FILE"
fi
...
export exec_lab_08="cd $QUESTIONS_DIR/question-008 && ./setup.sh"
...
export check_lab_08="cd $QUESTIONS_DIR/question-008 && ./validate.sh"
...
echo "     exec-lab 05    or  cd questions/question-005 && ./setup.sh"
    ...
    echo "     exec-lab 08    or  cd questions/question-008 && ./setup.sh"
...
echo "     check-lab 05   or  cd questions/question-005 && ./validate.sh"
    ...
    echo "     check-lab 08   or  cd questions/question-008 && ./validate.sh"
