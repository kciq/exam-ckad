...
exec-lab() {
    ...
    if [ ! -d "$lab_dir" ]; then
        echo "❌ Lab question-${lab_num#0} not found!"
        return 1
    fi
    
    echo "🚀 Setting up lab question-${lab_num#0}..."
    cd "$lab_dir"
    ./setup.sh
}
...
check-lab() {
    ...
    echo "🔍 Validating lab question-${lab_num#0}..."
    cd "$lab_dir"
    ./validate.sh
}
FUNCTIONS_EOF
...
EOF
        ...
        echo "ℹ️  Functions already in $SHELL_CONFIG"
    fi
fi
